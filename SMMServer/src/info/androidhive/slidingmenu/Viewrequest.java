			...
			public void run()
			{
				 try{            
			         ...
			         HttpPost   httppostt= new HttpPost("http://anythinginfotech.in/Android/club/server/upd_status.php");
			            ...
			            nameValuePairs11.add(new BasicNameValuePair("user_id",namee));  
                   ...
                   for(int i=0;i<arrayy.length();i++)
                    {
                  	 ...
                  	 runOnUiThread(new Runnable() 
                  	 {
			                public void run()
			                {
			                	...
			                	if(id1.equals("SUCCESS"))
			                	{
			                 		  Intent next=new Intent(getApplicationContext(),Enroll_MemberFragment.class);
			                 		 finish();
			                		  startActivity(next);
